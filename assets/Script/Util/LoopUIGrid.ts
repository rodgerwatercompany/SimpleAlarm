 ...
 */
@ccclass
export class LoopUIGrid extends cc.Component {
    ...
    public getChildList():cc.Node[] {
        const childCount:number = this.node.childrenCount;
        const childList:cc.Node[] = [];
        for (let index = 0; index < childCount; index++) {
            const child :cc.Node = this.node.children[index];
    ...
    public getChildByIndex(index:number):cc.Node {
        const childList:cc.Node[] = this.getChildList();
    ...
    private resetPosition(childList:cc.Node[]):void {
        ...
        for (let index = 0; index < length; index++) {
            const child:cc.Node = childList[index];
        ...
        if (this.pivot !== Pivot.TopLeft) {
            const po:cc.Vec2 = this.GetPivotOffset(this.pivot);
            let fx:number = 0;
            let fy:number = 0;
            ...
            for (let index = 0; index < length; index++) {
                const child:cc.Node = childList[index];
                const pos:cc.Vec2 = child.position;
    ...
    public getPreviewPostionByIndex(itemIndex:number):cc.Vec2 {
        ...
        const pos = cc.p((column) * this.cellWidth, (row) * -this.cellHeight);
